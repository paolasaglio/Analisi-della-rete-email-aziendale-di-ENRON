...
# --- 3. Bubble Chart: coerente con i dati sopra
p2_fine <- ggplot(result_dt, aes(x = year, y = num_communities)) +
  geom_point(aes(color = modularity, size = num_communities), alpha = 0.7) +
  scale_color_viridis_c(name = "Modularità", option = "viridis") +
  scale_size_area(name = "N° Comunità", max_size = 12) +
  scale_x_continuous(breaks = anni) +
  labs(
    title = "Analisi delle Comunità Email per Anno",
    subtitle = "Area bolla = numero comunità, colore = modularità",
    x = "Anno",
    y = "Numero di Comunità",
    caption = "Algoritmo: Infomap | Solo componente connessa maggiore"
  ) +
  theme_minimal() +
  theme(
    plot.background = element_rect(fill = "#262626", color = NA),
    panel.background = element_rect(fill = "#262626", color = NA),
    plot.title = element_text(size = 14, face = "bold", color = "#7F7F7F"),
    plot.subtitle = element_text(size = 12, color = "#7F7F7F"),
    plot.caption = element_text(color = "#7F7F7F"),
    axis.title = element_text(color = "white"),
    axis.text = element_text(color = "white"),
    ...
    panel.grid.minor = element_line(color = "#404040", size = 0.2),
...
# 3. GRAFICO CHE MOSTRA COME CAMBIA LA GRANDEZZA DELLE COMUNITA NEI MESI - UTILE!!!
library(igraph)
library(data.table)
library(ggplot2)
library(viridis)
...
# Creiamo la sequenza di mesi da gennaio 2000 a gennaio 2002
...
# Aggiungiamo una colonna con la data per la visualizzazione
...
# Opzione 1: Salva con ggsave (raccomandato)
